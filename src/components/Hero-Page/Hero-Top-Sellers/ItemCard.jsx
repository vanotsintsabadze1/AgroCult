...
function ItemCard({ itemData: { images, id, title, description, price } }) {
  return (
    <div className="flex h-full w-full snap-center items-center justify-center ">
      <div className="lg:row-gap shadow-soft relative flex h-[48rem] w-[38rem] flex-col gap-[1rem] rounded-[1rem] bg-white xs:w-[32rem] md:w-[35rem] lg:w-[35rem]">
        <div className="flex h-[20rem] w-full items-center justify-center rounded-tl-[1rem] rounded-tr-[1rem] p-[1rem]">
          <img src={images[0]} alt={`blog-${id}`} className="h-full max-w-full rounded-[1rem] object-fill" />
        </div>
        <div className="mt-[1rem] flex min-h-[10rem] w-full flex-col items-center gap-[1.5rem] text-center">
          ...
          <p className="line-clamp-3 w-[35rem] text-center text-[1.2rem] font-medium xs:w-[25rem] sm:w-[30rem] md:w-[30rem] lg:w-[30rem] lg:text-[1.3rem]">
            {description}...
          </p>
        </div>
