...
function ItemCard({ images, id, title, description, price, layout }: Props) {
  const multiColView = "flex max-w-[30rem] flex-col items-center rounded-lg bg-white p-[2rem] shadow-md ";
  const singleColView =
    "flex w-[32rem] flex-col items-center rounded-[1.5rem] bg-white p-[2rem] shadow-md lg:min-w-[60rem] xl:min-w-[80rem] lg:flex-row lg:gap-x-[2rem]";
      ...
      <div className="flex flex-col lg:gap-y-[3rem]">
        ...
        <div className="flex w-full justify-evenly">
          <button
            ...
            className={`min-w-[17rem] rounded-lg bg-green-700 py-[.7rem] text-[1.4rem] font-bold text-white`}
          >
            See Item
          </button>
          <button
